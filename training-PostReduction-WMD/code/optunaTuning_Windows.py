...
def save_best_model_and_embeddings_and_similarity(model, args, similarity_df):   
    ...
    # Define the file path for the best tuning similarity file
    similarity_file_dest = "output_of_model/evaluation/best_WMD_similarity.tsv"
    ...
    # Replacement of MeSH-terms in validation tokens with the corresponding MeSHIDs
    ...
    with open(embeddings_file_dest, "w") as embeddings_lock_file:
        ...
        # Generate and save Embeddings for the Best Validation Hyperparameter Set
        utilities.generate_document_embeddings(model, val_pmids, val_docs, embeddings_file_dest)
        # Release the file lock when done
    ...
    with open(similarity_file_dest, "w") as similarity_lock_file:
        ...
        similarity_df.rename(columns={"Cosine Similarity":"WMD Similarity"}, inplace=True)
        utilities.save_WMD_similarity_scores(similarity_df, similarity_file_dest)
        # Release the file lock when done
    ...
    def objective(trial):
        ...
        with precision_lock:
            if precision_5[0] > best_precision:
                ...
                save_best_model_and_embeddings_and_similarity(model, args, data)
                print('SAVINGGGGGGGG')
