...
**For Phase I, i.e. Hybrid-post(reduction)-Word2Doc2Vec-Doc-relevance and Hybrid-post(reduction)-WMD-Word2Vec-Doc-relevance which involve training using the entire Relish dataset, for a faster codebase—approximately 30% quicker—with simpler execution instructions and enhanced accuracy, employing an approach independent of preAnnot tokens for post-annotation, please refer to the directory [compact-code](./compact-code).**
