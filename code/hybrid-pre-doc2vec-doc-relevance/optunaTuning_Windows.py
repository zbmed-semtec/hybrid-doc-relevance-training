...
log_file = f"output_of_model/Optuna_trials_{args.classes}.log"
...
def save_best_model_and_embeddings_and_similarity(model, embeddings_df, similarity_df):
    ...
    model_file = f"output_of_model/model/best_Doc2Vec_model_{args.classes}"
    ...
    with open(model_file, "w") as model_lock_file:
        ...
        msvcrt.locking(model_lock_file.fileno(), msvcrt.LK_UNLCK, 0)

    # Define the file path for Storing Embeddings of the Best Validation Hyperparameter Set
    embeddings_file_dest = f"output_of_model/doc_embeddings/best_embeddings_pickle_{args.classes}.pkl"
    ...
    with open(embeddings_file_dest, "w") as embeddings_lock_file:
        ...
        # Release the file lock when done
        msvcrt.locking(embeddings_lock_file.fileno(), msvcrt.LK_UNLCK, 0)
     
    # Define the file path for the best similarity file
    similarity_file_dest = f"output_of_model/evaluation/best_cosine_similarity_{args.classes}.tsv"
    ...
    with open(similarity_file_dest, "w") as similarity_lock_file:
        ...
        # Release the file lock when done
        msvcrt.locking(similarity_lock_file.fileno(), msvcrt.LK_UNLCK, 0)
...
def objective_wrapper(args):
    def objective(trial):
        ...
        best_precision_path = f"output_of_model/best_precision_{args.classes}.txt"
...
def run_optuna_optimization(args, n_trials=10, n_jobs=1):
    ...
    study_storage = f'sqlite:///output_of_model/optuna_study_storage_{args.classes}.db'
    ...
    try:
        restored_sampler = pickle.load(open(f"output_of_model/optuna_sampler_{args.classes}.pkl", "rb"))
    ...
    study.trials_dataframe().to_csv(f'output_of_model/optuna_study_state_{args.classes}.csv')
    
    # Save the sampler with pickle to be loaded later.
    with open(f"output_of_model/optuna_sampler_{args.classes}.pkl", "wb") as fout:
