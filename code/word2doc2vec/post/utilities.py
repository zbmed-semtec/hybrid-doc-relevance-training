...
def injection_MeSHembeddings_into_embeddings(model: Word2Vec, pmids: str, article_doc: list, MeShIDtoPMID: str) -> Word2Vec:
    '''
...
def injection_MeSHIDs_into_tokens(pmids: str, article_doc: list, MeShIDtoPMID: str) -> list:
    '''
    ...
    Returns
    ----------
    article_doc: list of str
        Updated two dimensional list of all tokenized test/validation article documents (title + abstract).
    '''
...
def generate_document_embeddings(model: Word2Vec, pmids: list, article_doc: list) -> pd.DataFrame:
    '''
    ...
    for iteration in range(len(pmids)):
        ...
        # data_dict[pmids[iteration]] = 
        data_dict[pmids[iteration]] = {"word_count": word_count, "oov_words": missing_words}
        word_count = 0
    ...
    # df.index.name = 'pmid'
    flat_data = [{"pmid": pmid, "word_count": details["word_count"], "oov_words": details["oov_words"]}
                 for pmid, details in data_dict.items()]

    # Create a DataFrame from the list of dictionaries
    df = pd.DataFrame(flat_data)
    df.to_csv("post_w2d2v_min_count_1.tsv", sep='\t')
