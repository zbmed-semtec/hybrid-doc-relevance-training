...
def save_model_data(args, model, similarity):
    ...
    similarity_file = f"output_{args.classes}/validation/valid_wmd_similarity_{args.classes}.tsv"
...
def objective_wrapper(args, params):
    def objective(trial):
        ...
        similarity_df, model = run(params_dict, args, save_model=False) 
        ...
        with precision_lock:
            if precision_5[0] > best_precision:
                ...
                save_model_data(args, model, similarity_df)
...
def run_optuna_optimization(args, params, n_trials, n_jobs=1):
    ...
    study = optuna.create_study(direction='maximize', study_name="Hybrid_Post_WMD_Word2Vec_tuning", 
