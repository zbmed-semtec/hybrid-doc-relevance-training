...
This directory contains the code for the Hybrid-Post-fastText approach. This method leverages the plain text of the RELISH corpus, utilizes the fastText model to generate word embeddings, and calculates their centroid to produce document embeddings.
...
This step optimizes hyperparameters for a fastText model using Optuna, train the model with the optimal parameters, and evaluate its performance using precision at N (Precision@N) and normalized discounted cumulative gain (NDCG) metrics.

Pipeline Steps:
+ Hyperparameter Optimization: Utilizes Optuna to search for the best hyperparameters for the fastText model.
+ Model Training: Trains the fastText model with the optimal hyperparameters using the 90% training split data.
+ Embedding Generation: Generates embeddings for the 5% validation split data using the trained model.
...
```
python3 code/fasttext/postreduction/main.py -i data/Split_Dataset/Data/train.npy -t data/Split_Dataset/Data/test.npy -v data/Split_Dataset/Data/valid.npy -gt data/Split_Dataset/Ground_truth/test.tsv -gv data/Split_Dataset/Ground_truth/valid.tsv -dict data/mesh_to_pmid_dict.tsv -c 3 -win 0
```
