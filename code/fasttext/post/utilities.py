...
def create_fasttext_model(pmids: List[str], docs: List[List[str]], params: dict) -> FastText:
    ...
    print(f"Vocabulary size: {model.wv.vectors.shape}")
    return model
...
def injection_MeSHembeddings_into_embeddings(model: FastText, pmids: str, article_doc: list, MeShIDtoPMID: str) -> FastText:
    '''
...
def generate_document_embeddings(model: FastText, pmids: list, article_doc: list) -> pd.DataFrame:
    '''
    Generates document embeddings from a titles and abstracts in a given paper using fastText and calculating the cenroids 
